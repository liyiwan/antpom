...
public class AESUtils
{
    ...
    private static byte[] getRawKey(byte[] deskey) throws Exception
    {
        ...
        sr.setSeed(deskey);
        kgen.init(128, sr); // 生成密钥长度为128位bits （192 and 256 bits may not be available）
     ...
     * @throws Exception
     */
    private static byte[] encrypt(byte[] deskeyByte, byte[] encryptByte) throws Exception
    {
     ...
     * @throws Exception
     */
    private static byte[] decrypt(byte[] deskeyByte, byte[] encryptedByte) throws Exception
    {
        SecretKeySpec skeySpec = new SecretKeySpec(deskeyByte, "AES");
        Cipher cipher = Cipher.getInstance("AES");
    ...
    public static byte[] toByte(String encryptedStr)
    {
        ...
        for (int i = 0; i < len; i++)
            result[i] = Integer.valueOf(encryptedStr.substring(2 * i, 2 * i + 2), 16).byteValue();
