...
import com.yizi.iwuse.comm.handler.HandlerFactory;
...
public class CmdSendAdapter
{
    ...
    public static CmdResultInfo sendCmd(CmdInterface cmd, Object bean)
    {
        ...
        // 获取消息处理器
        AbsCmdMsgHandler msgHandler = HandlerFactory.getInstance().getCmdMsgHandlerByCmd(cmd);
        ...
        for (String scmd : interfaceCmd)
        {
            try
            {
                ...
                {
                    ILog.e(TAG, "send cmd:[" + scmd + "] ,cmd content is error,", e);
                }
    ...
    public static CmdResultInfo sendLogCmd(String fileName)
    {
        ...
        {
            // 如果没有定义logout命令，直接返回
            return connMgr.sendCmd(CmdInterface.PUBLIC_SendLogCmd.getCmds(CmdInterface.PUBLIC_SendLogCmd)[0], fileName);
        }
        catch (ConnException e)
        {
