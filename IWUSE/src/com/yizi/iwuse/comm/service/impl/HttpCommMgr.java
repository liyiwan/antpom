package com.yizi.iwuse.comm.service.impl;
...
import com.yizi.iwuse.comm.exception.ConnException;
import com.yizi.iwuse.comm.model.CmdResultInfo;
import com.yizi.iwuse.comm.model.ConnInfo;
import com.yizi.iwuse.comm.model.HttpConnInfo;
import com.yizi.iwuse.comm.service.AbsConnMgr;
...
public class HttpCommMgr extends AbsConnMgr
{
    ...
    public void initHttpsSSL() throws NoSuchAlgorithmException, KeyManagementException
    {
        // TLS验证
        ...
        // android 以前版本存在bug' it's doing an unnecessary DNS reverse lookup. 定义一个自定义的工厂类覆写其和dns解析相关的代码
    ...
    @Override
    public CmdResultInfo sendCmd(String cmdStr, String content) throws ConnException
    {
        if (null != content)
        {
    ...
    private CmdResultInfo sendHttpCommand(String json, String method)
    {
       ...
       final CmdResultInfo bean = new CmdResultInfo();
        ...
        try {
        	jsonObjectRequest = new JsonObjectRequest(info.getCmdUrl() + method, new JSONObject(json), new Response.Listener<JSONObject>(){
        		...
        		public void onResponse(JSONObject jsObj) {
                    ...
                    {
                    	bean.faultNo = PublicConst.RSP_JASON_ERR_CODE;
                    }
                    else
                    {
                        // 根据json消息中标志的success不为1，则将返回消息错误置为RSP_JASON_ERR_CODE
						if (PublicConst.RSP_MAILBOX_SUCCESS != jsObj.optInt("success"))
						{
							bean.faultNo = cmdRspParser._checkErrorAndException(jsObj, bean);
						}
                    }
                    bean.resInfo = jsObj;
        		}
...
//        			if (error. instanceof NetworkResponse)
//                    {
...
//                    }
        ...
        /**** 自己封装的http请求，用volley JsonObjectRequest方式取代
        HttpURLConnection conn = null;
        OutputStream out = null;
        try
        {
            if (!method.equals(CmdInterface.GetMail_BoxData.getCmds(CmdInterface.GetMail_BoxData)[0]))
            {
                ILog.v(TAG, "URL:" + info.getCmdUrl() + method);
            }
            conn = (HttpURLConnection) new URL(info.getCmdUrl() + method).openConnection();
            ...
            if (Thread.currentThread().getName().equalsIgnoreCase(PublicConst.MAIN_THREAD_FLAG))
            {
            ...
            conn.setRequestProperty("iwuseversion", AppContext.instance().iwuseVer
            ...
            // 如果是发送日志文件的
            if (method.equals(CmdInterface.PUBLIC_SendLogCmd.getCmds(CmdInterface.PUBLIC_SendLogCmd)[0]))
            {
    ...
    /**
     * 发送大文件
     * @param fileName 大文件内容
     * @param conn 当前的连接管理器
     * @throws IOException
     */
    public void sendBigFile(String fileName, HttpURLConnection conn) throws IOException
    {
     ...
     * @throws IOException
     */
    public void sendJsonString(String json, HttpURLConnection conn) throws IOException
    {
        OutputStream out = null;
        try
        {
    ...
    /**
     * 创建URL字符串
     * @param name 方法名或者是文件名
     * @param getFileFlag 是否是获取文件
     * 
     * @return
     */
    public String createURL(String name, boolean getFileFlag)
    {
        ...
        {
            // 如果是获取文件信息时的处理
            url.append(ipAddress).append(":").append(port).append("/").append(name).toString();
        }
        else
        {
            // 需要调用接口时的处理
            url.append(ipAddress).append(":").append(port).append("/action.cgi").append("?").append("ActionID")
