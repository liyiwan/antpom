...
import com.yizi.iwuse.comm.model.MailBoxMsgBean;
...
public abstract class AbsCmdMsgHandler
{
     ...
     * 
     * @param absCmd HID抽象的命令字
     * @param cmdStr 和设备对接的命令字符串
     * @param bean 包含命令参数的bean
     * @return 命令参数字符串，目前是json格式
     */
    public abstract String getSendMsdDetail(CmdInterface absCmd, String cmdStr, Object bean) throws JSONException;

    /**
     * 处理命令字的响应
     * @param cmd HID抽象的命令字
     * @param results 响应的结果对象集
     * @return
     */
    public abstract CmdResultInfo handleCmdRsp(CmdInterface cmd, List<CmdResultInfo> results);

    /**
     * 处理邮箱消息
     * 
     * @param mailMsgId 邮箱消息id
     * @param MailBoxMsgBean 邮箱消息每个msg的信息
     * @return 该邮箱消息处理后的结果
     */
    ...
    /**
     * 根据各个模块关系的邮箱订阅的字段进行相应的处理
     * 子类可以覆写该方法
     * @param json 邮箱消息中对应的'state'字段的json
     */
